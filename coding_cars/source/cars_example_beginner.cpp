...
String CarsExample::Beginner::OnInit(int& xCount, int& yCount, float& carSize, float& friction)
{
	xCount = 15;
	yCount = 15;
	carSize = 0.8;
...
void CarsExample::Beginner::OnCreateCar(Point& pos, float& angle360)
{
	pos.x = 3;
	pos.y = 7.5;
	angle360 = 90;
...
void CarsExample::Beginner::OnCrashCar(Point otherPos, Point& vec)
{
	vec.x += -otherPos.x / 10;
	vec.y += -otherPos.y / 10;
}

void CarsExample::Beginner::OnCrashTile(char tile, Wall wall, Point& vec)
{
	if(tile == 'o')
	{
		switch(wall)
...
void CarsExample::Beginner::OnInput(Joystick joystick, Point& vec, float& angle360)
{
...
void CarsExample::Beginner::OnRenderCar(ZayPanel& panel, float angle360, int fart)
{
...
void CarsExample::Beginner::OnRenderTile(ZayPanel& panel, char tile)
{
	if(tile == 'x')
